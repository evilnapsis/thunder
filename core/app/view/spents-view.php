...
<div class="panel panel-default">
  ...
  <a class='list-group-item' href="index.php?view=spents&opt=all"><?php if($category==null): ?><i class="glyphicon glyphicon-ok"></i> <?php endif; ?>Todas</a>
<?php foreach(CategoryData::getAll() as $cat):?>
  <a class='list-group-item' href="index.php?view=spents&opt=all&cat_id=<?php echo $cat->id; ?>"><?php if( $category!=null && ($category->id==$cat->id)): ?><i class="glyphicon glyphicon-ok"></i> <?php endif; ?><?php echo $cat->name; ?></a>
<?php endforeach; ?>
	</div>
</div>
</div>
  ...
  <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
		...
		<form class="form-horizontal" method="post" id="addproduct" action="index.php?action=spents&opt=add" role="form">
...
<script>
	$("#del-<?php echo $career->id?>").click(function(){
		...
		if(c==true){
			window.location = "index.php?action=spents&opt=del&id=<?php echo $career->id; ?>";
		}
	});
</script>
	</td>
