...
<?php
if(Core::$user->kind==1){
    $data["posts"]=InvestData::getAll();
}
else if(Core::$user->kind==2){
    $data["posts"]=InvestData::getAll();
}
else if(Core::$user->kind==3){
    $data["posts"]=InvestData::getAllBy("user_id",$_SESSION["user_id"]);
}
?>
...
<?php if(isset($_SESSION["aprove_item"])):?>
    ...
    <?php unset($_SESSION['aprove_item']); 
                                    ...
                                    <table class="table datatable table-bordered table-hover table-striped">
                                        ...
                                        <tbody>
                                        <?php foreach($data["posts"] as $postx):
                                        $post = InvoiceData::getById($postx->id);
                                            ...
                                            <tr>
                                                ...
                                                <td>$ <?=$postx->amount ;?></td>
                                                <td>$ <?=$postx->amount ;?></td>
                                                <td><?php if($postx->status_id==1):?>
                                                    <span class="badge text-bg-warning">Pendiente</span>
                                                    <?php elseif($postx->status_id==2):?>
                                                        <span class="badge text-bg-primary">Aprobado</span>
                                                        <?php elseif($postx->status_id==3):?>
                                                        <span class="badge text-bg-success">Finalizado</span>

                                                <?php endif; ?>
                                                <td><?php $user = UserData::getById($postx->user_id); echo $user->name; $user->lastname; ?></td>
                                                <td><?php echo $postx->created_at; ?></td>
                                                </td>
...
<?php if(Core::$user->kind==3):?>
    ...
    <?php endif; ?>
    <?php if(Core::$user->kind==1 || Core::$user->kind==2):?>
                                                <?php if($postx->status_id==1):?>
                                                    <a id="approveitem-<?=$postx->id;?>" class="btn btn-sm btn-primary"><i class="bi-check"></i> Aprobar</a>
<script type="text/javascript">
    $("#approveitem-<?php echo $post->id?>").click(function(){
Swal.fire({
  title: "Estas seguro que deseas Aprobar esta Inversion ?",
  ...
  if (result.isConfirmed) {
    window.location = "./?action=invests&opt=approve&id=<?=$postx->id;?>";
...
Swal.fire({
  ...
  /* Read more about isConfirmed, isDenied below */
  if (result.isConfirmed) {
    window.location = "./?action=invests&opt=del&id=<?=$post->id;?>";
  } else if(result.isDenied){
