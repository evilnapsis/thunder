...
class ReservationData {
	...
	public function add(){
		$sql = "insert into reservation (title,note,medic_id,date_at,time_at,pacient_id,user_id,status_id,sick,symtoms,medicaments,duration,bono,is_bono,created_at) ";
		$sql .= "value (\"$this->title\",\"$this->note\",\"$this->medic_id\",\"$this->date_at\",\"$this->time_at\",$this->pacient_id,$this->user_id,$this->status_id,\"$this->sick\",\"$this->symtoms\",\"$this->medicaments\",$this->duration,$this->bono,$this->is_bono,$this->created_at)";
...
// partiendo de que ya tenemos creado un objecto ReservationData previamente utilizamos el contexto
	public function update(){
		$sql = "update ".self::$tablename." set title=\"$this->title\",pacient_id=\"$this->pacient_id\",medic_id=\"$this->medic_id\",date_at=\"$this->date_at\",time_at=\"$this->time_at\",note=\"$this->note\",sick=\"$this->sick\",symtoms=\"$this->symtoms\",medicaments=\"$this->medicaments\",status_id=\"$this->status_id\",duration=\"$this->duration\" where id=$this->id";
		Executor::doit($sql);
	}
	...
	public static function getAllByDT($date,$time){
		$sql = "select * from ".self::$tablename." where date_at=\"$date\" and time_at=\"$time\"";
		$query = Executor::doit($sql);
		return Model::many($query[0],new ReservationData());
	}
